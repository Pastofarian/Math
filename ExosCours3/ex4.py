# 4. Implanter un filtre de flou linéaire avec Scipy ou votre programme par convolution
# avec un masque de 3x3 puis de 10x10 et appliquez-le à une image 512x512
...
def apply_blur_filter_scipy(image, filter_size):
    image_array = np.asarray(image)
    blur_filter = np.ones((filter_size, filter_size)) / (filter_size * filter_size)
    result = []
...
blur_3x3_button = tk.Button(
    button_frame, text="Blur 3x3", command=lambda: on_blur_button_click(3)
)
blur_3x3_button.pack(padx=(15, 15), pady=(15, 15))

blur_10x10_button = tk.Button(
    button_frame, text="Blur 10x10", command=lambda: on_blur_button_click(10)
)
