...
def apply_sharpening_filter(image):
    ...
    for channel in range(3):  # A modifier si en noir et blanc (1)
        result.append(
            convolve2d(image_array[:, :, channel], sharpen_filter, mode="same")
        )

    return np.stack(result, axis=-1).astype(np.uint8)


def on_sharpen_button_click():
    global matrice_img_array, photo2, canvas2

    # Applique le filtre de piqué
    sharpened_image = Image.fromarray(apply_sharpening_filter(mon_image))

    # Update l'image modifiée du canvas
    matrice_img_array_mod = np.array(sharpened_image)
    photo2 = ImageTk.PhotoImage(sharpened_image)
...
sharpen_button = tk.Button(
    button_frame, text="Sharpen", command=on_sharpen_button_click
)
sharpen_button.pack(padx=(15, 15), pady=(15, 15))
