# Exercice 15 : incrustation d’un logo dans une image
# Intégrez le logo de l’IFOSUP (ifosup-logo.png 211x55) dans une des images du répertoire à une distance
# de 20 pixels du bord supérieur de l’image et centrée.
# Variante : même chose mais sans afficher le fond blanc du logo, juste les lettres bleues
...
def add_logo():
    ...
    # Path des deux images
    main_image = Image.open("images/Lenna220.png")
    logo_image = Image.open("images/ifosup-logo.png").convert("RGBA")

    # On boucle pour remplacer le blanc de ifosup par une valeur alpha transparente
    ...
    # Position de l'image ifosup
    logo_x = (main_image.width - logo_image.width) // 2
    logo_y = 20

    # Colle l'image ifosup comme alpha sur Lenna
    main_image.paste(logo_image, (logo_x, logo_y), logo_image)

    # Affiche Lenna avec le logo ifosup
    result_photo = ImageTk.PhotoImage(main_image)
    canvas2.create_image(0, 0, anchor=tk.NW, image=result_photo)
    canvas2.image = result_photo
    canvas2.pack()
...
mix_button = tk.Button(button_frame, text="Mix", command=add_logo)
