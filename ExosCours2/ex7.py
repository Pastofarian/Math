# Exercice 7 : passez une image couleurs en image binaire noir et blanc
# Passer une image couleur en noir et blanc (2 couleurs seulement, image binaire) : chaque pixel ne peut
# prendre que la valeur 0 ou 255 pour ses trois composantes RGB.
# A vous d’essayer de définir une fonction ou une valeur de seuil qui fait basculer un pixel de l’image de
# départ en blanc ou en noir.
# Si vous créer des matrices de 0 de même taille que l’image originale, vous n’avez plus qu’à gérer les
# mises à 255 des pixels dépassant votre seuil.
...
def binary_black_white_filter(matrice_R, matrice_G, matrice_B):
    threshold = 50
    # average = (matrice_R + matrice_G + matrice_B) // 3
    # threshold = int(np.mean(average)) # calcule la valeur moyenne des intensités des pixels
    average = (matrice_R + matrice_G + matrice_B) // 3
    binary_R = np.where(average > threshold, 255, 0)
    binary_G = np.where(average > threshold, 255, 0)
    binary_B = np.where(average > threshold, 255, 0)
    mainLoop(binary_R, binary_G, binary_B)
...
bw_button = tk.Button(
    button_frame,
    text="B&W",
    command=lambda: binary_black_white_filter(matrice_R, matrice_G, matrice_B),
)
bw_button.config(foreground="black")
bw_button.pack(padx=(30, 30), pady=(15, 15))
