# Exercice 12 : image miroir horizontale ou verticale (flip)
# Implémentez un bouton "Flip". Ce bouton produira une symétrie axiale d'axe horizontal, ou, si vous
# préférez, ce qui est en haut va en bas et vice-versa : chaque pixel de la rangée 0 sera échangé avec celui
# en dessous de lui à la rangée pic.height()-1 ; ceux de la rangée 1 avec ceux de la rangée pic.height()-2, etc.
# On obtient au final une image correspondant au « reflet dans l’eau » de l’image de départ. (comparer avec
# un e double rotation de 90° de l’exercice ci-dessus).
# Variante : symétrie axiale d’axe vertical : ce qui est à gauche passe à droite (travail sur les colonnes à la
# place des lignes)
...
def flip_image(axis="horizontal"):
    global matrice_img_array_mod
    if axis == "horizontal":
        flipped_image = np.flip(matrice_img_array_mod, axis=0)
    elif axis == "vertical":
        flipped_image = np.flip(matrice_img_array_mod, axis=1)
    else:
        raise ValueError("Invalid axis, must be 'horizontal' or 'vertical'")

    mainLoop(flipped_image[:, :, 0], flipped_image[:, :, 1], flipped_image[:, :, 2])
...
flip_horizontal_button = tk.Button(
    button_frame, text="Flip Horizontal", command=lambda: flip_image("horizontal")
)
flip_horizontal_button.config(foreground="black")
flip_horizontal_button.pack(padx=(30, 30), pady=(15, 15))

flip_vertical_button = tk.Button(
    button_frame, text="Flip Vertical", command=lambda: flip_image("vertical")
)
flip_vertical_button.config(foreground="black")
flip_vertical_button.pack(padx=(30, 30), pady=(15, 15))
