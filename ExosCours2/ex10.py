# Exercice 10 : pixélisation d’une image
# Implémentez un bouton "Pixelize". L'idée est de diviser l'image en carrés de 10 pixels sur 10 (sur les bords,
# ce seront des rectangles). Dans un "carré", chaque composante de chaque pixel sera la valeur moyenne du
# carré. (n’ouliez pas qu’il existe une fonction numpy qui calcule automatiquement la valeur moyenne des
# valeurs d’un tableau et que vous devez travailler avec des entiers !)
...
def pixelize_filter(matrice_R, matrice_G, matrice_B, block_size=10):
    # On prend la shape de la matrice originale
    height, width = matrice_R.shape

    # On calcule la nouvelle height et width
    new_height = height // block_size * block_size
    new_width = width // block_size * block_size

    # On redimensionne les matrices pour qu'elles soient divisibles par la taille du bloc
    matrice_R = matrice_R[:new_height, :new_width]
    matrice_G = matrice_G[:new_height, :new_width]
    matrice_B = matrice_B[:new_height, :new_width]

    # redimensionne pour faire la moyenne
    matrice_R = matrice_R.reshape(
        new_height // block_size, block_size, new_width // block_size, block_size
    )
    matrice_G = matrice_G.reshape(
        new_height // block_size, block_size, new_width // block_size, block_size
    )
    matrice_B = matrice_B.reshape(
        new_height // block_size, block_size, new_width // block_size, block_size
    )

    # On calcule la moyenne de chaque block en int
    matrice_R = np.mean(matrice_R, axis=(1, 3)).astype(np.uint8)
    matrice_G = np.mean(matrice_G, axis=(1, 3)).astype(np.uint8)
    matrice_B = np.mean(matrice_B, axis=(1, 3)).astype(np.uint8)

    # On répète les valeurs moyennes pour créer l'effet pixelisé
    matrice_R = np.repeat(np.repeat(matrice_R, block_size, axis=0), block_size, axis=1)
    matrice_G = np.repeat(np.repeat(matrice_G, block_size, axis=0), block_size, axis=1)
    matrice_B = np.repeat(np.repeat(matrice_B, block_size, axis=0), block_size, axis=1)

    # On rempli les matrices pour correspondre aux dimensions d'origine
    pad_height = height - new_height
    pad_width = width - new_width

    matrice_R = np.pad(matrice_R, ((0, pad_height), (0, pad_width)), mode="edge")
    matrice_G = np.pad(matrice_G, ((0, pad_height), (0, pad_width)), mode="edge")
    matrice_B = np.pad(matrice_B, ((0, pad_height), (0, pad_width)), mode="edge")

    mainLoop(matrice_R, matrice_G, matrice_B)
...
pixelize_button = tk.Button(
    button_frame,
    text="Pixelize",
    command=lambda: pixelize_filter(matrice_R, matrice_G, matrice_B),
)
pixelize_button.config(foreground="black")
pixelize_button.pack(padx=(30, 30), pady=(15, 15))
